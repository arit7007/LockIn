...
- `supabase/functions/ai/index.ts` — the Edge Function `askAI()` calls. Verifies the caller's JWT via `supabase.auth.getUser()`, then proxies to OpenAI's Responses API (`gpt-5.4-mini` by default, overridable via an `OPENAI_MODEL` secret) using the `OPENAI_API_KEY` secret (`supabase secrets set`, never in frontend config) — same provider/model/endpoint the legacy Cloudflare Worker used, just relocated. Accepts `{prompt, image?}` (image is a base64 data URL sent as an `input_image` content part for vision requests), returns `{text}` on success or `{error}` on failure — the client always does its own JSON-extraction/parsing (`extractJSON`) on the returned text, so the function itself stays a thin, uniform "call the model" endpoint regardless of whether the caller wants a study plan, reels, coach feedback, or a schedule-photo class extraction.
